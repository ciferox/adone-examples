adone.app.run({
    main() {
        ...
        const lorem = adone.std.fs.readFileSync(`${__dirname}/data/git.diff`, "utf8");

        const cleanSides = screen.cleanSides;
        const expectClean = (value) => {
        ...
        */
        ...
        const btext = new adone.cli.ui.widget.Element({
            parent: screen,
            left: "center",
            top: "center",
            width: "80%",
        ...
        text.scroll = function (offset, always) {
            ...
            if (el.left + el.width > screen.width) {
                value = true;
            }
            if (el.top + el.height > screen.height) {
                value = false;
            }
            expectClean(value);
